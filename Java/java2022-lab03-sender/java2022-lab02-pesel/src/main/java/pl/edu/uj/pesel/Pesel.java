...
public class Pesel {
    ...
    public static boolean check(Pesel pesel) {
        ...
        int controlSum = 0; // DONE //calculate controlSum
        int[] controlSumScales = {1, 3, 7, 9, 1, 3, 7, 9, 1, 3, 1};
        for (int i = 0; i < pesel.pesel.length() - 1; i++) {
            controlSum += controlSumScales[i] * Integer.valueOf(String.valueOf(pesel.pesel.charAt(i)));
        }
